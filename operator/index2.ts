...
Status: ${contractApproved}`;
    ...
    console.log(`Signing and responding to task ${taskIndex} with ${message}`);
...
const registerOperator = async () => {
    ...
    if (isOperator)  {
        console.log("Already registered as operator in EigenLayer");
        return
    }
...
const monitorNewTasks = async () => {
    ...
    helloWorldServiceManager.on("TaskResponded", async (taskIndex: number, task: any, operator: string) => {
        console.log(`---Task Responded. Approval rate: ${taskIndex}%, ${task.name}, ${operator}---`);
    });
